...
class GetClubFormServiceTest : BehaviorSpec({
    ...
    val service = GetClubFormServiceImpl(
        clubFormRepository,
    ...
    fun field(id: Long, order: Int, type: ClubFormFieldType = ClubFormFieldType.TEXT, required: Boolean = false) =
        ClubFormFieldJpaEntity(id = id, form = form, label = "필드$id", description = null, fieldType = type, fieldOrder = order, isRequired = required)

    fun option(id: Long, field: ClubFormFieldJpaEntity, optionOrder: Int) =
        ClubFormFieldOptionJpaEntity(id = id, field = field, label = "옵션$id", value = "value$id", optionOrder = optionOrder)

    beforeTest {
        clearMocks(clubFormRepository, clubFormFieldRepository, clubFormFieldOptionRepository)
    }
